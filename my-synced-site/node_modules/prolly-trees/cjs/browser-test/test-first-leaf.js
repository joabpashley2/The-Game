'use strict';

var assert = require('assert');
var map = require('../src/map.js');
var codec = require('@ipld/dag-cbor');
var sha2 = require('multiformats/hashes/sha2');
var cache$1 = require('../src/cache.js');
var utils = require('../src/utils.js');

function _interopNamespace(e) {
  if (e && e.__esModule) return e;
  var n = Object.create(null);
  if (e) {
    Object.keys(e).forEach(function (k) {
      if (k !== 'default') {
        var d = Object.getOwnPropertyDescriptor(e, k);
        Object.defineProperty(n, k, d.get ? d : {
          enumerable: true,
          get: function () { return e[k]; }
        });
      }
    });
  }
  n["default"] = e;
  return Object.freeze(n);
}

var codec__namespace = /*#__PURE__*/_interopNamespace(codec);

const chunker = utils.bf(3);
const cache = cache$1.nocache;
const storage = () => {
  const blocks = {};
  const put = block => {
    blocks[block.cid.toString()] = block;
  };
  const get = async cid => {
    const block = blocks[cid.toString()];
    if (!block) {
      throw new Error('Not found');
    }
    return block;
  };
  return {
    get,
    put,
    blocks
  };
};
const opts = {
  cache,
  chunker,
  codec: codec__namespace,
  hasher: sha2.sha256
};
const createList = entries => entries.map(([key, value]) => ({
  key,
  value
}));
const list = createList([
  [
    'a',
    1
  ],
  [
    'b',
    1
  ],
  [
    'bb',
    2
  ],
  [
    'c',
    1
  ],
  [
    'cc',
    2
  ],
  [
    'd',
    1
  ],
  [
    'ff',
    2
  ],
  [
    'h',
    1
  ],
  [
    'z',
    1
  ],
  [
    'zz',
    2
  ]
]);
describe('map first-leaf', () => {
  it('minimal test case with leftmost key insertion', async () => {
    const {get, put} = storage();
    let mapRoot;
    const initialList = [{
        key: 'original',
        value: 1
      }];
    for await (const node of map.create({
        get,
        compare: utils.simpleCompare,
        list: initialList,
        ...opts
      })) {
      await put(await node.block);
      mapRoot = node;
    }
    const {result: initialResult} = await mapRoot.get('original');
    assert.deepStrictEqual(initialResult, 1);
    const bulk = [{
        key: 'before',
        value: 2
      }];
    const {blocks, root} = await mapRoot.bulk(bulk);
    for (const block of blocks) {
      await put(block);
    }
    mapRoot = root;
    const {result: newResult} = await mapRoot.get('before');
    assert.deepStrictEqual(newResult, 2);
    const {result: updatedResult} = await mapRoot.get('original');
    assert.deepStrictEqual(updatedResult, 1);
  });
  it('minimal test case with leftmost key insertion and custom always-split chunker', async () => {
    const {get, put} = storage();
    let mapRoot;
    const initialList = [{
        key: 'original',
        value: 1
      }];
    for await (const node of map.create({
        get,
        compare: utils.simpleCompare,
        list: initialList,
        ...opts
      })) {
      await put(await node.block);
      mapRoot = node;
    }
    const {result: initialResult} = await mapRoot.get('original');
    assert.deepStrictEqual(initialResult, 1);
    const alwaysSplitChunker = async (entry, distance) => {
      return true;
    };
    const bulk = [{
        key: 'before',
        value: 2
      }];
    const {blocks, root} = await mapRoot.bulk(bulk, {
      ...opts,
      chunker: alwaysSplitChunker
    });
    for (const block of blocks) {
      await put(block);
    }
    mapRoot = root;
    const {result: newResult} = await mapRoot.get('before');
    assert.deepStrictEqual(newResult, 2);
    const {result: updatedResult} = await mapRoot.get('original');
    assert.deepStrictEqual(updatedResult, 1);
  });
  it('big map with inner nodes', async () => {
    const {get, put} = storage();
    let mapRoot;
    for await (const node of map.create({
        get,
        compare: utils.simpleCompare,
        list,
        ...opts
      })) {
      await put(await node.block);
      mapRoot = node;
    }
    const keys = Array.from({ length: 100 }, (_, i) => String.fromCharCode(i + 32));
    for (const key of keys) {
      const bulk = [{
          key,
          value: key.charCodeAt(0)
        }];
      const {blocks, root} = await mapRoot.bulk(bulk);
      await Promise.all(blocks.map(block => put(block)));
      mapRoot = root;
      const {result} = await mapRoot.get(key).catch(e => {
        throw Error('Couldn\'t find key: ' + key);
      });
      assert.deepStrictEqual(result, key.charCodeAt(0));
    }
    const randomKey = keys[Math.floor(Math.random() * keys.length)];
    await mapRoot.getEntry(randomKey);
    await mapRoot.getAllEntries();
    const someKeys = keys.slice(20, 30);
    await mapRoot.getEntries(someKeys);
    const startKey = keys[10];
    const endKey = keys[20];
    await mapRoot.getRangeEntries(startKey, endKey);
  });
  it('basic numeric string key with specific keys and no loops inline', async () => {
    const {get, put} = storage();
    let mapRoot;
    const list = createList([
      [
        'b',
        1
      ],
      [
        'bb',
        2
      ],
      [
        'c',
        1
      ],
      [
        'cc',
        2
      ]
    ]);
    for await (const node of map.create({
        get,
        compare: utils.simpleCompare,
        list,
        ...opts
      })) {
      await put(await node.block);
      mapRoot = node;
    }
    const {result} = await mapRoot.get('c');
    assert.deepStrictEqual(result, 1);
    await mapRoot.get('a').then(() => {
      throw new Error('Key should not exist');
    }).catch(e => {
      assert.deepStrictEqual(e.message, 'Not found');
    });
    const everything0 = await mapRoot.getAllEntries();
    assert.deepStrictEqual(everything0.result.length, 4);
    const key2 = 'a';
    const value2 = `32-${ key2 }`;
    const bulk2 = [{
        key: key2,
        value: value2
      }];
    const {
      blocks: blocks2,
      root: root2
    } = await mapRoot.bulk(bulk2);
    for (const block of blocks2) {
      await put(block);
    }
    await put(root2.block);
    const address = await root2.address;
    const gotRoot = await get(address);
    mapRoot = root2;
    assert.deepStrictEqual(gotRoot.value, root2.block.value);
    const everything1 = await mapRoot.getAllEntries();
    assert.deepStrictEqual(everything1.result.length, 5);
    await mapRoot.get(key2).then(val => {
      assert.deepStrictEqual(val.result, value2);
    }).catch(e => {
      throw e;
    });
  });
  it('minimal nonreproduction of duplicate CID issue', async () => {
    const {get, put} = storage();
    let mapRoot;
    const list = createList([
      [
        'b',
        1
      ],
      [
        'c',
        1
      ]
    ]);
    for await (const node of map.create({
        get,
        compare: utils.simpleCompare,
        list,
        ...opts
      })) {
      await put(await node.block);
      mapRoot = node;
    }
    const key2 = 'a';
    const value2 = `32-${ key2 }`;
    const bulk2 = [{
        key: key2,
        value: value2
      }];
    const {
      blocks: blocks2,
      root: root2
    } = await mapRoot.bulk(bulk2);
    for (const block of blocks2) {
      await put(block);
    }
    await put(root2.block);
    const address = await root2.address;
    const gotRoot = await get(address);
    assert.deepStrictEqual(gotRoot.value, root2.block.value);
    assert.deepStrictEqual(root2.entryList.entries.length, 2);
    mapRoot = root2;
    const everything1 = await mapRoot.getAllEntries();
    assert.deepStrictEqual(everything1.result.length, 3);
    await mapRoot.get(key2).then(val => {
      assert.deepStrictEqual(val.result, value2);
    }).catch(e => {
      throw e;
    });
  });
  it('minimal reproduction of duplicate CID issue', async () => {
    const {get, put} = storage();
    let mapRoot;
    const list = createList([
      [
        'b',
        1
      ],
      [
        'c',
        1
      ],
      [
        'd',
        1
      ]
    ]);
    for await (const node of map.create({
        get,
        compare: utils.simpleCompare,
        list,
        ...opts
      })) {
      await put(await node.block);
      mapRoot = node;
    }
    const key2 = 'a';
    const value2 = `32-${ key2 }`;
    const bulk2 = [{
        key: key2,
        value: value2
      }];
    const {
      blocks: blocks2,
      root: root2
    } = await mapRoot.bulk(bulk2);
    for (const block of blocks2) {
      await put(block);
    }
    await put(root2.block);
    const address = await root2.address;
    const gotRoot = await get(address);
    assert.deepStrictEqual(gotRoot.value, root2.block.value);
    assert.deepStrictEqual(root2.entryList.entries.length, 2);
    mapRoot = root2;
    const everything1 = await mapRoot.getAllEntries();
    assert.deepStrictEqual(everything1.result.length, 4);
    await mapRoot.get(key2).then(val => {
      assert.deepStrictEqual(val.result, value2);
    }).catch(e => {
      throw e;
    });
  });
  it('minimal test case', async () => {
    const {get, put} = storage();
    let mapRoot;
    for await (const node of map.create({
        get,
        compare: utils.simpleCompare,
        list: list,
        ...opts
      })) {
      await put(await node.block);
      mapRoot = node;
    }
    const limit = 100;
    for (let rowCount = 76; rowCount < limit; rowCount++) {
      const key = String.fromCharCode(rowCount);
      const value = `${ rowCount }-${ key }`;
      const bulk = [{
          key,
          value
        }];
      const {blocks, root} = await mapRoot.bulk(bulk);
      for (const block of blocks) {
        await put(block);
      }
      mapRoot = root;
      const got = await mapRoot.get(key).catch(e => e);
      assert.deepStrictEqual(got.result, value, `failed at ${ value } w/ ${ JSON.stringify(got.result) }`);
    }
  });
  it('next test case two', async () => {
    const {get, put} = storage();
    let mapRoot;
    for await (const node of map.create({
        get,
        compare: utils.simpleCompare,
        list: list,
        ...opts
      })) {
      await put(await node.block);
      mapRoot = node;
    }
    const limit = 200;
    for (let rowCount = 77; rowCount < limit; rowCount++) {
      const key = String.fromCharCode(rowCount);
      const value = `${ rowCount }-${ key }`;
      const bulk = [{
          key,
          value
        }];
      const {blocks, root} = await mapRoot.bulk(bulk);
      for (const block of blocks) {
        await put(block);
      }
      mapRoot = root;
      const got = await mapRoot.get(key).catch(e => e);
      assert.deepStrictEqual(got.result, value, `failed at ${ value } w/ ${ JSON.stringify(got.result) }`);
    }
  });
  it('basic numeric string key', async () => {
    const {get, put} = storage();
    let mapRoot;
    for await (const node of map.create({
        get,
        compare: utils.simpleCompare,
        list,
        ...opts
      })) {
      await put(await node.block);
      mapRoot = node;
    }
    const {result} = await mapRoot.get('c').catch(e => {
      assert.deepStrictEqual(e.message, 'Failed at key: c');
    });
    assert.deepStrictEqual(result, 1);
    const errors = [];
    const limit = 500;
    for (let rowCount = 33; rowCount < limit; rowCount++) {
      const key = String.fromCharCode(rowCount);
      const value = `${ rowCount }-${ key }`;
      const bulk = [{
          key,
          value
        }];
      const {blocks, root} = await mapRoot.bulk(bulk);
      for (const bl of blocks) {
        await put(bl);
      }
      mapRoot = root;
      await mapRoot.get(key).then(() => {
      }).catch(e => {
        errors.push({
          key,
          value,
          rowCount
        });
      });
    }
    assert.deepStrictEqual(errors.length, 0);
  }).timeout(10000);
  it('basic decreasing string key', async () => {
    const {get, put} = storage();
    let mapRoot;
    for await (const node of map.create({
        get,
        compare: utils.simpleCompare,
        list,
        ...opts
      })) {
      await put(await node.block);
      mapRoot = node;
    }
    const {result} = await mapRoot.get('c').catch(e => {
      assert.deepStrictEqual(e.message, 'Failed at key: c');
    });
    assert.deepStrictEqual(result, 1);
    const bigLim = 102;
    for (let rowCount = bigLim; rowCount > 33; rowCount--) {
      const key = '' + rowCount;
      const value = `${ rowCount }-${ key }`;
      const bulk = [{
          key,
          value
        }];
      const {blocks, root} = await mapRoot.bulk(bulk);
      for (const bl of blocks) {
        await put(bl);
      }
      mapRoot = root;
      const got = await mapRoot.get(key);
      assert.deepStrictEqual(got.result, value);
      for await (const line of mapRoot.vis()) {
        assert.deepStrictEqual(typeof line, 'string');
      }
      const allE = await mapRoot.getAllEntries();
      assert.deepStrictEqual(allE.result.length, 11 + bigLim - rowCount);
    }
  }).timeout(10000);
  it('big decreasing string key normal', async () => {
    const {get, put} = storage();
    let mapRoot;
    for await (const node of map.create({
        get,
        compare: utils.simpleCompare,
        list,
        ...opts
      })) {
      await put(await node.block);
      mapRoot = node;
    }
    const {result} = await mapRoot.get('c').catch(e => {
      assert.deepStrictEqual(e.message, 'Failed at key: c');
    });
    assert.deepStrictEqual(result, 1);
    const bulk = [];
    const bigLim = 111;
    for (let rowCount = bigLim; rowCount > 33; rowCount--) {
      const key = '' + rowCount;
      const value = `${ rowCount }-${ key }`;
      bulk.push({
        key,
        value
      });
      const {blocks, root} = await mapRoot.bulk(bulk);
      for (const bl of blocks) {
        await put(bl);
      }
      mapRoot = root;
      const got = await mapRoot.get(key);
      assert.deepStrictEqual(got.result, value);
      for await (const line of mapRoot.vis()) {
        assert.deepStrictEqual(typeof line, 'string');
      }
      const allE = await mapRoot.getAllEntries();
      assert.deepStrictEqual(allE.result.length, 11 + bigLim - rowCount);
    }
  }).timeout(20000);
  it('insert causes chunker to return true for non-empty bulk', async () => {
    const {get, put} = storage();
    let root;
    for await (const node of map.create({
        get,
        compare: utils.simpleCompare,
        list,
        ...opts
      })) {
      await put(await node.block);
      root = node;
    }
    const chunker = async (node, context) => {
      if (context.height === 1 && node.bulk.length > 0) {
        return true;
      }
      return false;
    };
    const keyToInsert = 'x';
    const valueToInsert = 42;
    const {
      blocks,
      root: updatedRoot,
      previous
    } = await root.bulk([{
        key: keyToInsert,
        value: valueToInsert
      }], { chunker });
    await Promise.all(blocks.map(block => put(block)));
    assert.deepStrictEqual(previous, []);
    const {result} = await updatedRoot.get(keyToInsert);
    assert.deepStrictEqual(result, valueToInsert);
    for (const {key, value} of list) {
      const {result} = await updatedRoot.get(key);
      assert.deepStrictEqual(result, value);
    }
  });
  it('inserts with uninitialized node', async () => {
    const {get, put} = storage();
    let root;
    for await (const node of map.create({
        get,
        compare: utils.simpleCompare,
        list,
        ...opts
      })) {
      await put(await node.block);
      root = node;
    }
    const keysToInsert = [
      '1',
      '2',
      '3'
    ];
    const bulk = keysToInsert.map(key => ({
      key,
      value: parseInt(key)
    }));
    const {
      blocks,
      root: updatedRoot
    } = await root.bulk(bulk, {
      ...opts,
      chunker
    });
    await Promise.all(blocks.map(block => put(block)));
    for (const key of keysToInsert) {
      const value = await updatedRoot.get(key);
      assert.deepStrictEqual(value.result, parseInt(key));
    }
    const {result: entries} = await updatedRoot.getAllEntries();
    const resultingKeys = entries.map(({key}) => key);
    resultingKeys.sort(utils.simpleCompare);
    const expectedKeys = [
      'a',
      'b',
      'cc',
      'c',
      'bb',
      'd',
      'ff',
      'h',
      'z',
      'zz',
      '1',
      '2',
      '3'
    ].sort(utils.simpleCompare);
    assert.deepStrictEqual(resultingKeys, expectedKeys);
  });
  it('insert multiple entries within range causing chunker function to return true', async () => {
    const initialList = [
      {
        key: 'a',
        value: 1
      },
      {
        key: 'b',
        value: 1
      },
      {
        key: 'bb',
        value: 2
      },
      {
        key: 'c',
        value: 1
      },
      {
        key: 'cc',
        value: 2
      },
      {
        key: 'd',
        value: -1
      },
      {
        key: 'ff',
        value: 2
      },
      {
        key: 'h',
        value: 1
      },
      {
        key: 'z',
        value: 1
      },
      {
        key: 'zz',
        value: 2
      }
    ];
    const {get, put} = storage();
    let root;
    for await (const node of map.create({
        get,
        compare: utils.simpleCompare,
        list: initialList,
        ...opts
      })) {
      await put(await node.block);
      root = node;
    }
    const bulkInserts = [
      {
        key: 'aa',
        value: 3
      },
      {
        key: 'ca',
        value: 4
      },
      {
        key: 'da',
        value: 5
      }
    ];
    const customChunker = async (entry, distance) => {
      return distance > 0;
    };
    const {
      blocks,
      root: newRoot
    } = await root.bulk(bulkInserts, {
      ...opts,
      chunker: customChunker
    });
    await Promise.all(blocks.map(block => put(block)));
    const _get = async k => (await newRoot.get(k)).result;
    assert.deepStrictEqual(await _get('aa'), 3);
    assert.deepStrictEqual(await _get('ca'), 4);
    assert.deepStrictEqual(await _get('da'), 5);
    const expected = [
      [
        'a',
        1
      ],
      [
        'aa',
        3
      ],
      [
        'b',
        1
      ],
      [
        'bb',
        2
      ],
      [
        'c',
        1
      ],
      [
        'ca',
        4
      ],
      [
        'cc',
        2
      ],
      [
        'd',
        -1
      ],
      [
        'da',
        5
      ],
      [
        'ff',
        2
      ],
      [
        'h',
        1
      ],
      [
        'z',
        1
      ],
      [
        'zz',
        2
      ]
    ];
    for (const [key, value] of expected) {
      assert.deepStrictEqual(await _get(key), value);
    }
    const {result: allEntries} = await newRoot.getAllEntries();
    const actualKeys = allEntries.map(entry => entry.key);
    const expectedKeys = expected.map(([key]) => key);
    assert.deepStrictEqual(actualKeys, expectedKeys);
  });
  it('test case to trigger the uncovered branch in mergeFirstLeftEntries', async () => {
    const {get, put} = storage();
    let mapRoot;
    const initialList = [{
        key: 'A',
        value: 1
      }];
    for await (const node of map.create({
        get,
        compare: utils.simpleCompare,
        list: initialList,
        ...opts
      })) {
      await put(await node.block);
      mapRoot = node;
    }
    const {result: initialResult} = await mapRoot.get('A');
    assert.deepStrictEqual(initialResult, 1);
    const alwaysSplitChunker = async (entry, distance) => {
      return distance < 100;
    };
    const bulk = [
      {
        key: 'B',
        value: 2
      },
      {
        key: 'C',
        value: 3
      },
      {
        key: 'D',
        value: 4
      },
      {
        key: 'F',
        value: 6
      },
      {
        key: 'G',
        value: 7
      },
      {
        key: 'H',
        value: 8
      }
    ];
    const {blocks, root} = await mapRoot.bulk(bulk, {
      ...opts,
      chunker: alwaysSplitChunker
    });
    for (const block of blocks) {
      await put(block);
    }
    mapRoot = root;
    const newBulk = [{
        key: 'E',
        value: 5
      }];
    const newOpts = {
      ...opts,
      chunker: async (entry, distance) => {
        return distance < 100;
      }
    };
    const {
      blocks: newBlocks,
      root: newRoot
    } = await mapRoot.bulk(newBulk, newOpts);
    for (const block of newBlocks) {
      await put(block);
    }
    mapRoot = newRoot;
    const {result: newResult} = await mapRoot.get('E');
    assert.deepStrictEqual(newResult, 5);
    const {result: updatedResultA} = await mapRoot.get('A');
    assert.deepStrictEqual(updatedResultA, 1);
    const {result: updatedResultB} = await mapRoot.get('B');
    assert.deepStrictEqual(updatedResultB, 2);
    const {result: updatedResultC} = await mapRoot.get('C');
    assert.deepStrictEqual(updatedResultC, 3);
    const {result: updatedResultD} = await mapRoot.get('D');
    assert.deepStrictEqual(updatedResultD, 4);
    const {result: updatedResultF} = await mapRoot.get('F');
    assert.deepStrictEqual(updatedResultF, 6);
    const {result: updatedResultG} = await mapRoot.get('G');
    assert.deepStrictEqual(updatedResultG, 7);
    const {result: updatedResultH} = await mapRoot.get('H');
    assert.deepStrictEqual(updatedResultH, 8);
  });
  it('loopless shorter decreasing logic branch in getAllEntries', async () => {
    const {get, put} = storage();
    let mapRoot;
    const initialList = [{
        key: '2000',
        value: 1
      }];
    for await (const node of map.create({
        get,
        compare: utils.simpleCompare,
        list: initialList,
        ...opts
      })) {
      await put(await node.block);
      mapRoot = node;
    }
    const index = 1;
    const key = (2000 - index).toString();
    const bulk = [{
        key,
        value: index
      }];
    const {blocks, root} = await mapRoot.bulk(bulk, { ...opts });
    for (const block of blocks) {
      await put(block);
    }
    const got = await root.get(key);
    assert.deepStrictEqual(got.result, index);
    mapRoot = root;
    const got2 = await mapRoot.get(key).catch(err => ({ err }));
    assert.deepStrictEqual(undefined, got2.err);
    assert.deepStrictEqual(got2.result, index);
    const {result: allEntries} = await mapRoot.getAllEntries();
    assert.deepStrictEqual(allEntries.length, 2, 'Unexpected number of entries retrieved');
  });
  it('test case to loop decreasing logic branch in getAllEntries with alphabetic keys', async () => {
    const {get, put} = storage();
    let mapRoot;
    const initialList = [{
        key: 'f',
        value: 1
      }];
    for await (const node of map.create({
        get,
        compare: utils.simpleCompare,
        list: initialList,
        ...opts
      })) {
      await put(await node.block);
      mapRoot = node;
    }
    const keys = [
      'e',
      'd',
      'c',
      'b',
      'a'
    ];
    const size = keys.length;
    for (let index = 0; index < size; index++) {
      const key = keys[index];
      const bulk = [{
          key,
          value: index + 1
        }];
      const {blocks, root} = await mapRoot.bulk(bulk, { ...opts });
      await put(await root.block);
      for (const block of blocks) {
        await put(block);
      }
      const got = await root.get(key);
      assert.deepStrictEqual(got.result, index + 1);
      mapRoot = root;
    }
    for (let index = 0; index < size; index++) {
      const key = keys[index];
      const got = await mapRoot.get(key).catch(err => ({ err }));
      assert.deepStrictEqual(undefined, got.err);
      assert.deepStrictEqual(got.result, index + 1);
    }
    const {result: allEntries} = await mapRoot.getAllEntries();
    assert.deepStrictEqual(allEntries.length, size + 1, 'Unexpected number of entries retrieved');
  });
  it('test case to loop decreasing logic branch in getAllEntries og', async () => {
    const {get, put} = storage();
    let mapRoot;
    const initialList = [{
        key: '2000',
        value: 1
      }];
    for await (const node of map.create({
        get,
        compare: utils.simpleCompare,
        list: initialList,
        ...opts
      })) {
      await put(await node.block);
      mapRoot = node;
    }
    const size = 50;
    for (let index = 1; index < size; index++) {
      const key = (2000 - index).toString();
      const bulk = [{
          key,
          value: index
        }];
      const {blocks, root} = await mapRoot.bulk(bulk, { ...opts });
      for (const block of blocks) {
        await put(block);
      }
      const got = await root.get(key);
      assert.deepStrictEqual(got.result, index);
      mapRoot = root;
    }
    for (let index = 1; index < size; index++) {
      const key = (2000 - index).toString();
      const got = await mapRoot.get(key).catch(err => ({ err }));
      assert.deepStrictEqual(undefined, got.err);
      assert.deepStrictEqual(got.result, index);
    }
    const {result: allEntries} = await mapRoot.getAllEntries();
    assert.deepStrictEqual(allEntries.length, size, 'Unexpected number of entries retrieved');
  });
  it('test case to trigger increasing logic branch in getAllEntries', async () => {
    const {get, put} = storage();
    let mapRoot;
    const initialList = [{
        key: '2000',
        value: 1
      }];
    for await (const node of map.create({
        get,
        compare: utils.simpleCompare,
        list: initialList,
        ...opts
      })) {
      await put(await node.block);
      mapRoot = node;
    }
    const size = 50;
    for (let index = 0; index < size; index++) {
      const key = (1000 + index).toString();
      const bulk = [{
          key,
          value: index
        }];
      const {blocks, root} = await mapRoot.bulk(bulk, { ...opts });
      for (const block of blocks) {
        await put(block);
      }
      const got = await root.get(key);
      assert.deepStrictEqual(got.result, index);
      mapRoot = root;
    }
    for (let index = 0; index < size; index++) {
      const key = (1000 + index).toString();
      const got = await mapRoot.get(key);
      assert.deepStrictEqual(got.result, index);
    }
    const {result: allEntries} = await mapRoot.getAllEntries();
    assert.deepStrictEqual(allEntries.length, size + 1, 'Unexpected number of entries retrieved');
  });
  it('inserts with custom chunker that triggers uncovered branches', async () => {
    const {get, put} = storage();
    let root;
    for await (const node of map.create({
        get,
        compare: utils.simpleCompare,
        list,
        ...opts
      })) {
      await put(await node.block);
      root = node;
    }
    const customChunker = async (entry, distance) => {
      return distance > 1;
    };
    const bulkInserts = [
      {
        key: 'a',
        value: 10
      },
      {
        key: 'c',
        value: 20
      },
      {
        key: 'h',
        value: 30
      },
      {
        key: 'z',
        value: 40
      }
    ];
    const {
      blocks,
      root: newRoot
    } = await root.bulk(bulkInserts, {
      ...opts,
      chunker: customChunker
    });
    await Promise.all(blocks.map(block => put(block)));
    const _get = async k => (await newRoot.get(k)).result;
    assert.deepStrictEqual(await _get('a'), 10);
    assert.deepStrictEqual(await _get('c'), 20);
    assert.deepStrictEqual(await _get('h'), 30);
    assert.deepStrictEqual(await _get('z'), 40);
    const expected = [
      [
        'a',
        10
      ],
      [
        'b',
        1
      ],
      [
        'bb',
        2
      ],
      [
        'c',
        20
      ],
      [
        'cc',
        2
      ],
      [
        'd',
        1
      ],
      [
        'ff',
        2
      ],
      [
        'h',
        30
      ],
      [
        'z',
        40
      ],
      [
        'zz',
        2
      ]
    ];
    for (const [key, value] of expected) {
      assert.deepStrictEqual(await _get(key), value);
    }
    const {result: allEntries} = await newRoot.getAllEntries();
    const actualKeys = allEntries.map(entry => entry.key);
    const expectedKeys = expected.map(([key]) => key);
    assert.deepStrictEqual(actualKeys, expectedKeys);
  });
});
